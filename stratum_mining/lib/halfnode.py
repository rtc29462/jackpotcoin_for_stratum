...
class CBlock(object):
    ...
    def __repr__(self):
        if settings.COINDAEMON_ALGO == 'jackpotcoin':
            return "CBlock(nVersion=%i hashPrevBlock=%064x hashMerkleRoot=%064x nTime=%s nBits=%08x nNonce=%08x nSuperBlock=%08x nRoundMask=%08x vtx=%s)" % (self.nVersion, self.hashPrevBlock, self.hashMerkleRoot, time.ctime(self.nTime), self.nBits, self.nNonce, self.nSuperBlock, self.nRoundMask, repr(self.vtx))
        else:
